 ...
 *  >0  -  nal length
*/
int32_t parse_nal(uint8_t * buf, uint32_t len, int32_t *startcode_len)
{
//    for(int i=0; i<16; i++)
//    {
//        printf("%2x ", buf[i]);
...
{
    uint32_t read_len = 0;
    int32_t parse_len = 0, startcode_len = 0;
    ...
    if (payload_type == RTP_PAYLOAD_H264) {
        if (offset > 0) {
            ...
            if (parse_len > startcode_len) {
                ...
                //todo:优化拷贝
                uint8_t *tmp_buf = new uint8_t[RTP_READ_FILE_BYTES * 10];
                memcpy(tmp_buf, buf_read + parse_len, offset);
                memcpy(buf_read, tmp_buf, offset);
                delete tmp_buf;
